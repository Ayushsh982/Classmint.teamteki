...
<div class="container mt-5 mb-5">
    <h2 class="mb-4 fw-bold">JEE ADVANCED 2026 Core Track</h2>
    <?php 
    $jeeadvance_data = [
        'Physics' => [
            'Class 11th' => ['Mechanics', 'Thermal Physics', 'Waves and Acoustics'],
            'Class 12th' => ['Electromagnetism', 'Optics', 'Modern Physics']
        ],
        'Chemistry' => [
            'Class 11th' => ['Physical Chemistry Core', 'Inorganic Chemistry Basics', 'Organic Chemistry Principles'],
            'Class 12th' => ['Advanced Physical Chemistry', 'Transition Elements', 'Advanced Organic Reactions']
        ],
        'Mathematics' => [
            'Class 11th' => ['Algebra', 'Trigonometry', 'Coordinate Geometry'],
            'Class 12th' => ['Differential Calculus', 'Integral Calculus', 'Vectors and 3D Geometry']
        ]
    ];

    foreach ($jeeadvance_data as $subject => $grades) {
        ...
        foreach ($grades as $grade => $chapters) {
            ...
            if (empty($chapters)) {
                ...
                foreach ($chapters as $chapter) {
                    ...
                    echo '<a href="quiz.php?topic='.urlencode($chapter).'&exam=jeeadvance" class="btn btn-primary btn-sm mt-1">Start Test </a>';
                    echo '</div></div>';
                }
            }
            echo '</div></div></div>';
